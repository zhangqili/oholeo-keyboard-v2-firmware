...
+ 支持最多5层按键绑定
+ 支持切换6键无冲与全键无冲模式
+ 支持模拟DInput游戏摇杆与按键，可通过Steam重映射
+ 支持模拟XInput游戏手柄，可免映射使用
+ 支持模拟MIDI键盘
+ 支持硬件录制播放按键宏
+ 配套免驱配置器
+ 支持Windows动态光效(实验性，需要在keyboard_conf.h中取消注释LIGHTING_ENABLE，可能会干扰ADC采样)
+ 支持调用脚本(实验性，需要在keyboard_conf.h中取消注释SCRIPT_ENABLE)
+ 支持PC调用MTP接口通过文件管理器读写板载Flash文件(实验性，需要在keyboard_conf.h中取消注释MTP_ENABLE)
...
如果已经烧录了该键盘固件，在键盘上电时按住旋钮或按住Esc键进入Bootloader，在键盘正常运行时按住再松开BOOT键进入bootloader。\
...
|`FN`+`DEL`+`B`|切换使能蜂鸣器|
|`FN`+`DEL`+`M`|切换使能电磁阀|
|`FN`+`DEL`+`N`|恢复默认状态，取消使能蜂鸣器，取消使能电磁阀，开启全键无冲|
|`FN`+`DEL`+`R`|重启|
